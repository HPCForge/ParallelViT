...
using cutlass_tensorop_s1688tf32gemm_grouped_256x128_32x3_tt_align4_base =
  typename cutlass::gemm::kernel::DefaultGemmGrouped<
    ...
    cutlass::epilogue::thread::LinearCombinationGeneric<cutlass::epilogue::thread::GELU, float, 4, float, float>,
...
using DeviceKernel = cutlass::gemm::device::GemmGrouped<cutlass_tensorop_s1688tf32gemm_grouped_256x128_32x3_tt_align4_base>;
...
cutlass::Status linearGelu_kernel_run(int problem_count, cutlass::gemm::GemmCoord* problem_sizes,
...
std::vector<at::Tensor> linearGelu_kernel(const std::vector<at::Tensor>& A, const std::vector<at::Tensor>& B, at::optional<const std::vector<at::Tensor>> C, float alpha, float beta) {
    size_t num = A.size();
    ...
    cutlass::Status status = linearGelu_kernel_run(
        num,
